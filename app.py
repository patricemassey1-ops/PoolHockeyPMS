...
def is_admin_user() -> bool:
    """Admin = Whalers (mais l'accès NE DOIT PAS dépendre de l'équipe sélectionnée).
    - Si l'utilisateur a déjà été reconnu admin une fois dans la session, on garde le flag (is_admin_cached=True)
      pour éviter les 'kick' vers Home lors des reruns.
    - On ne 'désactive' pas admin quand il choisit une autre équipe pour consulter.
    """
    ...
    # Reconnaître admin quand Whalers a été sélectionné au moins une fois
    if owner_eff in {"whalers"}:
        st.session_state["is_admin_cached"] = True

    # Admin effectif = cache (ne dépend PAS de l'équipe courante)
    return bool(st.session_state.get("is_admin_cached", False))
